...
class WatchlistAPISource: WatchlistDataSource {
    ...
    override fun createWatchlist(
        ...
        token: String,
        callback: (ApiResponse<MovieModel>) -> Unit
    ) {
        ...
        val createWatchlistCall: Call<WatchlistModel> = api.createWatchlist(
            WatchlistModel(
                watchlistName, "", "",
                listOf(), listOf()
            ), token
    ...
    override fun read_db(token: String, callback: (ApiResponse<List<WatchlistModel>>) -> Unit) {
        ...
        getAllWatchlistsCall.enqueue(object : Callback<List<WatchlistModel>> {
            override fun onResponse(
                call: Call<List<WatchlistModel>>,
                response: Response<List<WatchlistModel>>
            ) {
                if (response.isSuccessful) {
    ...
    override fun getWatchlistById(
        watchlistId: String,
        token: String,
        callback: (ApiResponse<WatchlistModel>) -> Unit
    ) {
        val api = retrofit.create(WatchlistAPI::class.java)

        val getWatchlistByIdCall: Call<WatchlistModel> = api.getWatchlistById(
            watchlistId, token
        )
